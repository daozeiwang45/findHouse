...
- (void)cardViewDefaultScale
{
    ...
    for (int i = 0; i < _currentViews.count; i++) {
        ...
        if (i == 1) {
            view.transform = CGAffineTransformIdentity;
            view.frame = CGRectMake(_defaultFrame.origin.x, _defaultFrame.origin.y - kCard_Margin, _defaultFrame.size.width, _defaultFrame.size.height); //change 2015.12.28
            view.transform = CGAffineTransformScale(CGAffineTransformIdentity,kSecondCard_Scale,kSecondCard_Scale);
        }
        if (i == 2) {
            view.transform = CGAffineTransformIdentity;
            view.frame = CGRectMake(_defaultFrame.origin.x, _defaultFrame.origin.y - (kCard_Margin * 2), _defaultFrame.size.width, _defaultFrame.size.height); // change 2015.12.28
            view.transform = CGAffineTransformScale(CGAffineTransformIdentity,kTherdCard_Scale,kTherdCard_Scale);
        }
    }
}
