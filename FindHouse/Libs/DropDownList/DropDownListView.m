...
- (id)initWithFrame:(CGRect)frame WithColumnNames:(NSArray *)columnNames dataSource:(id)datasource delegate:(id) delegate
{
    ...
    if (self) {
        ...
        for (int i = 0; i <sectionNum; i++) {
            ...
            [sectionBtn  setTitle:columnNames[i] forState:UIControlStateNormal];
//            [sectionBtn setTitleColor:UIColor@"858585"] forState:UIControlStateNormal];
            [sectionBtn setTitleColor:UIColorFromRGB(0x858585) forState:UIControlStateNormal];
            ...
            if (i<sectionNum && i != 0) {
                ...
                lineView.backgroundColor = [UIColor colorWithRed:133/255.0 green:133/255.0 blue:133/255.0 alpha:0.3];
...
- (void)setUpSelectedArray
{
    self.selectedArray = [NSMutableArray array];
    for (int index = 0;index < [self.dropDownDataSource numberOfSections] ; index++) {
        NSString *num = @"0";
        [self.selectedArray addObject:num];
    }
}
...
-(void)showChooseListViewInSection:(NSInteger)section choosedIndex:(NSInteger)index
{
    if (!self.mTableView) {
        ...
        self.mTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 40) style:UITableViewStylePlain];
//        self.mTableView.tableFooterView = [[UIView alloc] init];
    ...
    //修改tableview的frame
    float sectionWidth = (float)(self.frame.size.width)/[self.dropDownDataSource numberOfSections];
    NSInteger rowNumber = rowNumber = [self.dropDownDataSource numberOfRowsInSection:section];;
...
//    (rowNumber * 40) < 240 ? (rowNumber * 40):240
    //动画设置位置
    rect .size.height = (rowNumber * 40) < 240 ? (rowNumber * 40):240;
    [UIView animateWithDuration:0.3 animations:^{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DropDownListViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [[DropDownListViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
//        cell.accessoryType = UITableViewCellAccessoryNone;
    }
    cell.arrorImage.hidden = YES;
    NSString *nowSelectedNum = [NSString stringWithFormat:@"%lu",indexPath.row];
    NSString *oldSelectedNum = self.selectedArray[currentExtendSection];
    if ([nowSelectedNum isEqualToString:oldSelectedNum]) {
        cell.arrorImage.hidden = NO;
    }
    cell.titleLbl.text = [self.dropDownDataSource titleInSection:currentExtendSection index:indexPath.row];
    cell.titleLbl.textColor= UIColorFromRGB(0x333333);
