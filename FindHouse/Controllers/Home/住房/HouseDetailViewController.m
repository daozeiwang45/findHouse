...
@interface HouseDetailViewController ()<SDCycleScrollViewDelegate,UIGestureRecognizerDelegate,UITableViewDataSource,UITableViewDelegate>
{
...
@property (nonatomic, strong) DetailView *header;
...
@property (nonatomic, strong) DetailView *detailView; // 中部详情区
...
//    _detailBgView.backgroundColor = [UIColor orangeColor];
    ...
    if (_houseKind == RentKind) {
        ...
        UIView *centerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kSCREEN_W, 845)];
        _detailView.frame = centerView.bounds;
        [centerView addSubview:_detailView];
        _tableView.tableHeaderView = centerView;
        _detailView.saleView.hidden = YES;
    }else if (_houseKind == SaleKind){
        
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kSCREEN_W, kSCREEN_H-135-64) style:UITableViewStylePlain];
        _detailView = [[[NSBundle mainBundle] loadNibNamed:@"DetailView" owner:self options:nil] lastObject];
        UIView *centerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kSCREEN_W, 845-48-37)];
        _detailView.frame = centerView.bounds;
        [centerView addSubview:_detailView];
        _tableView.tableHeaderView = centerView;
        _detailView.saleView.hidden = NO;
        _detailView.rentView.hidden = YES;
        _detailView.topToRent.priority = 250;
        _detailView.topToSale.priority = 999;
        _detailView.commentHc.constant = 0;
        _detailView.commentSection.hidden = YES;
