...
#import "SearchResultVController.h"
#import "HouseRecmdViewController.h"
#import "OneRentViewController.h"

@interface WannaRentViewController ()

@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (strong, nonatomic) Searchfooter *footer;
...
static NSString *cellID = @"rangeCell";
@implementation WannaRentViewController
...
- (void)viewDidLoad {
    ...
    [self _initUI];
    [self startLocation];
    [_tableView registerNib:[UINib nibWithNibName:@"PriceRangeCell" bundle:nil] forCellReuseIdentifier:cellID];
}
...
- (void)_initUI {
    ...
    _locService.delegate = self;
        //footer
    UIView *footView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kSCREEN_W, 272)];
    _footer = [[[NSBundle mainBundle] loadNibNamed:@"Searchfooter" owner:self options:nil] lastObject];
    _footer.frame = footView.bounds;
    [footView addSubview:_footer];
    _footer.lampIV.image = [UIImage imageNamed:@"lamp_yellow"];
    _footer.bottomIV.image = [UIImage imageNamed:@"bottom_yellow"];
    [_footer.oneRentBtn setImage:[UIImage imageNamed:@"oneRent"] forState:UIControlStateNormal];
    ...
    _footer.recommendLbl.userInteractionEnabled = YES;
    [_footer.recommendLbl addGestureRecognizer:searchTap];
    // 好房推荐lbl
    ...
    // 一键租房 按钮
    [_footer.oneRentBtn addTarget:self action:@selector(oneRentAction) forControlEvents:UIControlEventTouchUpInside];
    
}

  // 跳转查找结果页面
- (void)searchResultAction {
    SearchResultVController *seResultVC = [[SearchResultVController alloc] init];
    seResultVC.houseKind = RentKind;
    [self.navigationController pushViewController:seResultVC animated:YES];
}
  // 跳转好房推荐十套 页面
- (void)recmdAction {
    HouseRecmdViewController *recmdVC = [self.storyboard instantiateViewControllerWithIdentifier:@"recmdVC"];
    recmdVC.houseKind = RentKind;
    
    [self.navigationController pushViewController:recmdVC animated:YES];
}
  // 跳转一键租房页面
- (void)oneRentAction {
    OneRentViewController *oneRentVC = [self.storyboard instantiateViewControllerWithIdentifier:@"oneRent"];
    [self.navigationController pushViewController:oneRentVC animated:YES];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PriceRangeCell *cell = [_tableView dequeueReusableCellWithIdentifier:cellID forIndexPath:indexPath];
