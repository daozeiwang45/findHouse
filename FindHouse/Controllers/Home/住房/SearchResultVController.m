...
@interface SearchResultVController ()<UIGestureRecognizerDelegate>
{
    UIButton *btn,*preBtn;
}
...
static NSString *filterCell = @"houseFilterCell";
@implementation SearchResultVController
...
- (void)viewDidLoad {
    ...
    currentSection = -1;
    _sectionArr = [NSMutableArray arrayWithArray:@[@[@"最近",@"最远"],@[@"最高",@"最低"],@[@"一房",@"二房",@"三房",@"四房",@"五房"],@[@"奢华装",@"精装",@"中装",@"简装",@"毛坯"],@[@"独立阳台",@"独立厨房",@"院子",@"停车位"]]];
    NSArray *titles = @[@"距离",@"热度",@"装修",@"户型",@"更多"];
    CGSize imageSize=CGSizeZero, titleSize=CGSizeZero;
    for (int i=0 ; i<titles.count; i++) {
        btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake(kSCREEN_W/5*i, 64, kSCREEN_W/5, 50);
        btn.tag = i;
        ...
        if (i<titles.count && i != 0) {
            UIImageView *lineView = [[UIImageView alloc] initWithFrame:CGRectMake(kSCREEN_W/5*i-1, 64+btn.height/4.0, 1, btn.height/2)];
            lineView.image = [UIImage imageNamed:@"verticalline_dark"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == _tableView) {
        ...
        if (btnSection == 4) {
            HouseFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:filterCell];
            if (cell == nil) {
                cell = [[[NSBundle mainBundle] loadNibNamed:@"HouseFilterCell" owner:self options:nil] lastObject];
            }
            return cell;
